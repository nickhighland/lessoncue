...
async function prepareHostedTrivia(page: Page, name: string) {
  return page.evaluate(async activityName => {
    ...
    const pairing = await fetch("/api/v1/pairing/request", {
      method: "POST", headers,
      body: JSON.stringify({ deviceName: `TV ${activityName}`, platform: "android-tv", appVersion: "0.40.52" }),
    ...
    await fetch("/api/v1/tv/status", {
      ...
      body: JSON.stringify({
        screenId: identity.screenId, appVersion: "0.40.52", online: true, freeBytes: 4e9,
