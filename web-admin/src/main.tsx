...
type TemporaryControllerSession = {
  token: string;
  classId: string;
  lessonId?: string;
  expiresAt?: string;
...
class SignageEditorErrorBoundary extends Component<
...
function formTags(form: FormData) {
  return form
    .getAll("tags")
...
function RetentionChoices({ lessonDate }: { lessonDate: string }) {
  return (
    <fieldset className="retention-options">
...
function formatDateAfterDays(date: string, days: number) {
...
function dateInputValue(value?: string, addDays = 0) {
...
function parseStoredDates(value: string) {
  try {
...
function scheduleSummary(schedule: RecurringSchedule) {
...
function newSignageZone(type: SignageZoneType, content = ""): SignageZone {
  return {
    id: localId(),
    type,
...
function signageTime(value?: number, fallback = "") {
...
function lessonPlannedDurationMs(lesson: Lesson) {
  return lesson.items
    .filter((item) => item.role === "lesson")
    .reduce((sum, item) => sum + cuePlannedDurationMs(item), 0);
}
...
function parseDiagnosticJson<T>(value?: string): T[] {
  try {
    const parsed = JSON.parse(value || "[]");
...
function formatClockOffset(value?: number) {
...
function isConvertibleDocument(media: Media) {
  return (
...
function convertedSlideCount(media: Media) {
  try {
...
function friendlyPlaybackState(state?: string) {
  return (
    (
      {
        idle: "Ready",
...
function roleName(role: PlaylistItem["role"]) {
  return role === "preRoll"
    ? "PRE-ROLL"
    : role === "countdown"
      ? "COUNTDOWN"
...
function detectDuration(file: File): Promise<number | undefined> {
