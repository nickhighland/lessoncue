...
type HardwareAccelerationStatus = { supported: boolean; available: boolean; engine: string; message: string; lastCheckedAt?: string; lastHardwareUseAt?: string; lastFallbackAt?: string; lastError?: string };
...
function Settings({ bootstrap, backups, audit, refresh, notify, canSettings, canBackups, canUpdates }: { bootstrap: Bootstrap; backups: Backup[]; audit: Audit[]; refresh: () => void; notify: (s: string) => void; canSettings: boolean; canBackups: boolean; canUpdates: boolean }) {
  ...
  async function saveAdaptiveTranscoding(event: FormEvent<HTMLFormElement>) { event.preventDefault(); try { await api("/api/v1/organization", { method: "PUT", body: JSON.stringify({ ...o, adaptiveTranscodingEnabled: adaptiveTranscoding, transcodeLeadDays: Number(transcodeLeadDays), hardwareAccelerationEnabled: hardwareAcceleration }) }); refresh(); notify(hardwareAcceleration && bootstrap.hardwareAcceleration.available ? "Intel Quick Sync will accelerate local video conversion." : "Playback profile settings saved."); } catch (e) { notify(errorText(e)); } }
  async function checkHardware() { setCheckingHardware(true); try { const status = await api<HardwareAccelerationStatus>("/api/v1/hardware-acceleration/check", { method: "POST", body: "{}" }); refresh(); notify(status.available ? "Intel Quick Sync is ready." : status.message); } catch (e) { notify(errorText(e)); } finally { setCheckingHardware(false); } }
