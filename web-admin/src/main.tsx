...
type Permission = "planning.manage" | "uploads.manage" | "playback.control" | "screens.manage" | "users.manage" | "app-settings.manage" | "settings.manage" | "backups.manage" | "updates.manage";
...
type PlaylistItem = {
  ...
  fitMode: "fit" | "fill" | "letterbox"; rotationDegrees: number;
  cropLeftPercent: number; cropTopPercent: number; cropRightPercent: number; cropBottomPercent: number;
  muted: boolean; playbackRatePercent: number; repeatCount: number; imageDurationSeconds?: number;
  backgroundColor: string; transitionStyle: "cut" | "fade-black"; transitionDurationMs: number;
  flexibleTime: boolean;
};
type Lesson = {
  id: string; classId: string; className: string; date: string; title: string; designatedStartAt?: string; preRollStartsAt?: string;
  preRollEnabled: boolean; countdownItemId?: string; version: number; archived: boolean; keepOffline: boolean; downloadDaysBefore: number; volumePercent: number; muted: boolean; substituteNotes: string; preRollMonitorUrl?: string; generatedByScheduleId?: string; items: PlaylistItem[];
};
type LessonTemplate = {
  ...
  preRollEnabled: boolean; keepOffline: boolean; downloadDaysBefore: number; volumePercent: number; muted: boolean; scheduleCount: number;
...
type Screen = {
  ...
  signageOnly?: boolean; permanentPairing?: boolean;
...
const permissionOptions: { id: Permission; label: string; detail: string }[] = [
  ...
  { id: "app-settings.manage", label: "App settings", detail: "Registration, media taxonomy, pairing PINs, recycling, and activity" },
  { id: "settings.manage", label: "Service settings", detail: "Email, storage allocation, playback, network, remote access, and server setup" },
  { id: "backups.manage", label: "Privacy, backups, and restore", detail: "Create, download, validate, and restore backups" },
...
function Shell({ view, setView, username, currentUsername, role, permissions, onLogout, notice, setNotice }: {
  ...
  useEffect(() => {
    Promise.all([
      ...
      canManageAppSettings ? api<Audit[]>("/api/v1/audit") : Promise.resolve([]),
  ...
  }, [dataVersion, setNotice, canManageUsers, canManageBackups, canManageAppSettings]);
  useEffect(() => {
    if (!bootstrap) return;
  ...
  useEffect(() => {
    if (view === "controller" && !canControl || view === "classes" && !canPlan || view === "templates" && !canPlan ||
        ...
        view === "settings" && !canManageAppSettings && !canManageServiceSettings && !canManageBackups && !canManageUpdates) setView("dashboard");
  }, [view, canControl, canPlan, canManageUsers, canManageAppSettings, canManageServiceSettings, canManageBackups, canManageUpdates, setView]);
  ...
  const nav: [View, string, string][] = [["dashboard", "⌂", "Dashboard"],
    ...
    ...(canManageAppSettings || canManageServiceSettings || canManageBackups || canManageUpdates ? [["settings", "⚙", "Settings"]] as [View, string, string][] : [])];
  return <><a className="skip-link" href="#main-content">Skip to main content</a><div className={`app-shell ${view === "controller" ? "controller-mode" : ""}`}>
    ...
    <main className="content" id="main-content" tabIndex={-1}>
      ...
      {loading && !bootstrap ? <div className="loading">Loading local data…</div> : <>
        ...
        {view === "signage" && <SignageView signage={signage} media={media} screens={screens} timeZone={bootstrap?.timeZone || "UTC"} sourceAllowlist={parseStringArray(bootstrap?.settings.signageSourceAllowlistJson)} refresh={refresh} notify={setNotice} />}
        ...
        {view === "settings" && bootstrap && <Settings bootstrap={bootstrap} backups={backups} audit={audit} refresh={refresh} notify={setNotice} canAppSettings={canManageAppSettings} canServiceSettings={canManageServiceSettings} canBackups={canManageBackups} canUpdates={canManageUpdates} />}
      </>}
    </main>
...
function ClassesView({ classes, lessons, media, taxonomy, refresh, notify, canUpload, storage, localControllerOrigin }: { classes: LessonClass[]; lessons: Lesson[]; media: Media[]; taxonomy: MediaTaxonomy; refresh: () => void; notify: (s: string) => void; canUpload: boolean; storage?: StorageStatus; localControllerOrigin?: string }) {
  ...
  if (lesson) return <LessonEditor lesson={lesson} classes={classes} allLessons={lessons} media={media} taxonomy={taxonomy} onBack={() => setEditing(undefined)} refresh={refresh} notify={notify} canUpload={canUpload} storage={storage} />;
...
function LessonEditor({ lesson, classes, allLessons, media, taxonomy, onBack, refresh, notify, canUpload, storage }: { lesson: Lesson; classes: LessonClass[]; allLessons: Lesson[]; media: Media[]; taxonomy: MediaTaxonomy; onBack: () => void; refresh: () => void; notify: (s: string) => void; canUpload: boolean; storage?: StorageStatus }) {
  ...
  const [editingMode, setEditingMode] = useState<"simple" | "advanced">(() =>
    localStorage.getItem("lessoncue.lesson-editor.mode") === "advanced" ? "advanced" : "simple");
  ...
  const plannedDurationMs = lessonItems.reduce((total, item) => total + cuePlannedDurationMs(item), 0);
  const flexibleDurationMs = lessonItems.filter(item => item.flexibleTime).reduce((total, item) => total + cuePlannedDurationMs(item), 0);
  const plannedEnd = lesson.designatedStartAt ? new Date(new Date(lesson.designatedStartAt).getTime() + plannedDurationMs) : undefined;
  const conflicts = lesson.designatedStartAt ? allLessons.filter(other => other.id !== lesson.id && other.classId === lesson.classId && !other.archived && other.designatedStartAt &&
    intervalsOverlap(new Date(lesson.designatedStartAt!), plannedEnd || new Date(lesson.designatedStartAt!),
      new Date(other.designatedStartAt), new Date(new Date(other.designatedStartAt).getTime() + lessonPlannedDurationMs(other)))) : [];
  ...
  async function relocateLesson(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const values = Object.fromEntries(new FormData(event.currentTarget));
    try {
      await api<{ id: string }>(`/api/v1/lessons/${lesson.id}/relocate`, { method: "POST", body: JSON.stringify({ action: relocateAction, classId: values.classId, date: values.date, title: values.title }) });
      setShowRelocate(false); refresh(); notify(relocateAction === "copy" ? "Lesson copied with its complete run of show." : "Lesson moved to the selected class and date.");
      if (relocateAction === "move") onBack();
    } catch (error) { notify(errorText(error)); }
  }
  return <>
    <button className="back-button" onClick={onBack}>← Back to {lesson.className}</button>
    <PageHead eyebrow="LESSON BUILDER" title={lesson.title} detail={`${formatDate(lesson.date)} · Manifest version ${lesson.version}`} action={<div className="page-actions"><button className="button" onClick={() => setShowRunSheet(true)}>Print run sheet</button><button className="button" onClick={() => setShowRelocate(true)}>Copy or move</button>{canUpload && <button className="button primary" onClick={() => setShowAdd(true)}>Add media</button>}</div>} />
    <section className="editor-mode-switch" aria-label="Lesson editor mode"><div><strong>{editingMode === "simple" ? "Simple editing" : "Advanced editing"}</strong><small>{editingMode === "simple" ? "Everyday playback controls without technical detail." : "Crop, rotation, speed, repeats, precise timing, fades, and transitions."}</small></div><div role="group" aria-label="Choose editor mode"><button className={editingMode === "simple" ? "active" : ""} onClick={() => { setEditingMode("simple"); localStorage.setItem("lessoncue.lesson-editor.mode", "simple"); }}>Simple</button><button className={editingMode === "advanced" ? "active" : ""} onClick={() => { setEditingMode("advanced"); localStorage.setItem("lessoncue.lesson-editor.mode", "advanced"); }}>Advanced</button></div></section>
    ...
    {showRunSheet && <Modal title={`Run sheet: ${lesson.title}`} onClose={() => setShowRunSheet(false)}><LessonRunSheet lesson={lesson} /><div className="modal-actions no-print"><button className="button" onClick={() => setShowRunSheet(false)}>Close</button><button className="button primary" onClick={() => window.print()}>Print run sheet</button></div></Modal>}
    {showRelocate && <Modal title="Copy or move lesson" onClose={() => setShowRelocate(false)}><form className="stack" onSubmit={relocateLesson}><fieldset className="retention-options"><legend>What should happen?</legend><label><input type="radio" checked={relocateAction === "copy"} onChange={() => setRelocateAction("copy")} /><span><strong>Copy lesson</strong><small>Keep the original and duplicate its complete playlist, notes, and timing.</small></span></label><label><input type="radio" checked={relocateAction === "move"} onChange={() => setRelocateAction("move")} /><span><strong>Move lesson</strong><small>Move this lesson and shift its scheduled times to the new date.</small></span></label></fieldset><div className="two-fields"><Field label="Destination class"><select name="classId" defaultValue={lesson.classId} required>{classes.map(item => <option value={item.id} key={item.id}>{item.name}</option>)}</select></Field><Field label="Destination date"><input name="date" type="date" defaultValue={lesson.date} required /></Field></div><Field label="Lesson title"><input name="title" defaultValue={relocateAction === "copy" ? `${lesson.title} copy` : lesson.title} maxLength={160} required key={relocateAction} /></Field><button className="button primary">{relocateAction === "copy" ? "Create copy" : "Move lesson"}</button></form></Modal>}
    <div className="editor-grid">
      <section className="panel schedule-panel"><h2>Lesson settings</h2><form className="stack" onSubmit={updateLesson}><Field label="Lesson title"><input name="title" defaultValue={lesson.title} required /></Field><Field label="Lesson date"><input name="date" type="date" defaultValue={lesson.date} required /></Field><Field label="Pre-roll begins" hint="Paired screens automatically start looping pre-roll at this time."><input name="preRollStartsAt" type="datetime-local" defaultValue={toLocalInput(lesson.preRollStartsAt)} /></Field><Field label="Designated class start" hint="The countdown begins exactly one countdown-video duration before this time."><input name="designatedStartAt" type="datetime-local" defaultValue={toLocalInput(lesson.designatedStartAt)} /></Field><Field label="Substitute or teacher instructions" hint="Shown on the phone controller and printed run sheet, never on the audience display."><textarea name="substituteNotes" rows={4} maxLength={8000} defaultValue={lesson.substituteNotes} placeholder="Room setup, handoff details, or what a substitute should know" /></Field><Field label="Optional pre-roll livestream monitor" hint="During pre-roll, the phone controller can display this HTTP/HTTPS camera or stream page. It is never sent to the audience display."><input name="preRollMonitorUrl" type="url" maxLength={2000} defaultValue={lesson.preRollMonitorUrl || ""} placeholder="https://camera.example.org/view" /></Field><div className="two-fields"><Field label="Whole-lesson volume"><div className="unit-input"><input name="volumePercent" type="number" min="0" max="150" defaultValue={lesson.volumePercent ?? 100} /><span>%</span></div></Field><label className="check-card"><input type="checkbox" name="muted" defaultChecked={lesson.muted} /><span><strong>Mute whole lesson</strong><small>All cue audio stays silent.</small></span></label></div><label className="switch-row"><input type="checkbox" name="preRollEnabled" defaultChecked={lesson.preRollEnabled} /><span /><div><strong>Enable pre-roll</strong><small>Loop all pre-roll items until the countdown or class begins.</small></div></label><button className="button primary">Save lesson settings</button></form>
        <div className="run-timing-summary"><div><span>ESTIMATED RUN TIME</span><strong>{formatFriendlyDuration(plannedDurationMs)}</strong><small>{flexibleDurationMs ? `${formatFriendlyDuration(flexibleDurationMs)} marked flexible` : "No flexible-time cues"}</small></div><div><span>PLANNED FINISH</span><strong>{plannedEnd ? plannedEnd.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) : "Set a start time"}</strong><small>{lessonItems.some(item => cuePlannedDurationMs(item) === 0) ? "Unknown-duration cues are not included" : `${lessonItems.length} main cues`}</small></div></div>
        {plannedEnd && plannedEnd.getTime() < Date.now() && <div className="alert warning">This lesson's planned finish has passed. Check the live controller for the current remaining time.</div>}
        {conflicts.length > 0 && <div className="alert error"><strong>Schedule conflict:</strong> {conflicts.map(item => `${item.className} — ${item.title}`).join(", ")} overlaps this lesson's estimated window.</div>}
      ...
      <section className="panel playlist-panel"><div className="panel-heading"><div><h2>Playback sequence</h2><p>Pre-roll loops, countdown runs once, then lesson media plays in order.</p></div><span className="pill">{items.length} items</span></div>
        ...
        {items.length ? <div className="playlist"><label className="playlist-select-all"><input type="checkbox" checked={allCuesSelected} onChange={toggleAllCues} /> Select all cues</label>{items.map((item, index) => <PlaylistCueRow key={item.id} item={item} index={index} total={items.length} advanced={editingMode === "advanced"} selected={selectedCueIds.has(item.id)} onSelected={() => toggleCue(item.id)} onMove={move} onChange={changeItem} onTimeline={() => setPreviewItem(item)} onRemove={removeItem} />)}</div> : <Empty title="This playlist is empty" body="Add videos, audio, or images from your local media library." action={<button className="button primary" onClick={() => setShowAdd(true)}>Add media</button>} />}
      </section>
    </div>
  </>;
}

function PlaylistCueRow({ item, index, total, advanced, selected, onSelected, onMove, onChange, onTimeline, onRemove }: {
  ...
  advanced: boolean; selected: boolean; onSelected: () => void; onTimeline: () => void; onRemove: (id: string) => void | Promise<void>;
}) {
  ...
  return <article className={`playlist-item ${item.role} ${selected ? "selected" : ""}`}>
    ...
    <div className="item-main"><div><span className={`role ${item.role}`}>{roleName(item.role)}</span><strong>{item.title}</strong></div><small>{item.mediaFileName || item.type} · {formatDuration(item.durationMs || item.mediaDurationMs)}</small>
      <button className="timeline-edit" onClick={onTimeline}>▥ Visually trim both ends & edit fades</button>
      <div className="item-options simple-cue-controls"><Field label="Role"><select aria-label="Role" value={item.role} onChange={e => onChange(item, { role: e.target.value })}><option value="preRoll">Pre-roll</option><option value="countdown">Countdown</option><option value="lesson">Main lesson</option></select></Field><Field label="At the end"><select aria-label="End behavior" value={item.endBehavior} onChange={e => onChange(item, { endBehavior: e.target.value })}><option value="advance">Play next cue</option><option value="loop">Loop continuously</option><option value="pause">Pause on final frame</option><option value="stop">Stop playback</option></select></Field><Field label="Cue volume"><div className="unit-input"><input type="number" min="0" max="150" defaultValue={item.volumePercent} onBlur={e => onChange(item, { volumePercent: Number(e.target.value) })} /><span>%</span></div></Field>{visual && <Field label="Picture"><select aria-label="Picture fit" value={item.fitMode || "fit"} onChange={e => onChange(item, { fitMode: e.target.value })}><option value="fit">Fit on screen</option><option value="fill">Fill screen (crop edges)</option><option value="letterbox">Letterbox on black</option></select></Field>}{item.type === "image" && <Field label="Still/slide duration"><div className="unit-input"><input type="number" min="1" max="3600" defaultValue={item.imageDurationSeconds || 10} onBlur={e => onChange(item, { imageDurationSeconds: Number(e.target.value) })} /><span>sec</span></div></Field>}<label className="compact-check"><input type="checkbox" checked={item.muted} onChange={e => onChange(item, { muted: e.target.checked })} /> Mute cue</label><label className="compact-check"><input type="checkbox" checked={item.flexibleTime} onChange={e => onChange(item, { flexibleTime: e.target.checked })} /> Flexible timing</label></div>
      <Field label="Teacher / volunteer notes" hint="Shown beside this cue on the phone controller and printed run sheet."><textarea rows={2} maxLength={2000} defaultValue={item.notes || ""} placeholder="What the operator should say or do" onBlur={e => e.target.value !== item.notes && onChange(item, { notes: e.target.value })} /></Field>
      {visual && <div className="cue-appearance-summary"><i style={{ background: item.backgroundColor || "#000000" }} /><span>{item.fitMode || "fit"}{item.rotationDegrees ? ` · rotated ${item.rotationDegrees}°` : ""}{item.transitionStyle === "fade-black" ? ` · ${(item.transitionDurationMs / 1000).toFixed(1)}s fade through black` : " · cut"}</span></div>}
      {advanced && <details className="item-advanced" open><summary>Advanced cue controls</summary><div className="advanced-grid"><Field label="Display title"><input defaultValue={item.title} onBlur={e => e.target.value !== item.title && onChange(item, { title: e.target.value })} /></Field><Field label="Playback speed"><div className="unit-input"><input type="number" min="25" max="400" step="5" defaultValue={item.playbackRatePercent || 100} onBlur={e => onChange(item, { playbackRatePercent: Number(e.target.value) })} /><span>%</span></div></Field><Field label="Play count before ending"><input type="number" min="1" max="99" defaultValue={item.repeatCount || 1} onBlur={e => onChange(item, { repeatCount: Number(e.target.value) })} /></Field>{visual && <><Field label="Rotate"><select value={item.rotationDegrees || 0} onChange={e => onChange(item, { rotationDegrees: Number(e.target.value) })}><option value="0">No rotation</option><option value="90">90° clockwise</option><option value="180">180°</option><option value="270">270° clockwise</option></select></Field><Field label="Background"><input type="color" value={item.backgroundColor || "#000000"} onChange={e => onChange(item, { backgroundColor: e.target.value })} /></Field><Field label="Transition"><select value={item.transitionStyle || "cut"} onChange={e => onChange(item, { transitionStyle: e.target.value })}><option value="cut">Cut</option><option value="fade-black">Fade through black</option></select></Field>{item.transitionStyle === "fade-black" && <Field label="Transition duration"><div className="unit-input"><input type="number" min="0" max="5" step="0.1" defaultValue={(item.transitionDurationMs || 500) / 1000} onBlur={e => onChange(item, { transitionDurationMs: Math.round(Number(e.target.value) * 1000) })} /><span>sec</span></div></Field>}<Field label="Crop left"><div className="unit-input"><input type="number" min="0" max="45" defaultValue={item.cropLeftPercent || 0} onBlur={e => onChange(item, { cropLeftPercent: Number(e.target.value) })} /><span>%</span></div></Field><Field label="Crop right"><div className="unit-input"><input type="number" min="0" max="45" defaultValue={item.cropRightPercent || 0} onBlur={e => onChange(item, { cropRightPercent: Number(e.target.value) })} /><span>%</span></div></Field><Field label="Crop top"><div className="unit-input"><input type="number" min="0" max="45" defaultValue={item.cropTopPercent || 0} onBlur={e => onChange(item, { cropTopPercent: Number(e.target.value) })} /><span>%</span></div></Field><Field label="Crop bottom"><div className="unit-input"><input type="number" min="0" max="45" defaultValue={item.cropBottomPercent || 0} onBlur={e => onChange(item, { cropBottomPercent: Number(e.target.value) })} /><span>%</span></div></Field></>}<Field label="Start at (seconds)"><input type="number" min="0" step="0.04" defaultValue={item.startMs / 1000} onBlur={e => onChange(item, { startMs: Math.round(Number(e.target.value) * 1000) })} /></Field><Field label="End at (seconds)"><input type="number" min="0" step="0.04" defaultValue={item.endMs ? item.endMs / 1000 : ""} onBlur={e => onChange(item, e.target.value ? { endMs: Math.round(Number(e.target.value) * 1000) } : { clearEndMs: true })} /></Field><Field label="Fade in (seconds)"><input type="number" min="0" max="30" step="0.1" defaultValue={(item.fadeInMs || 0) / 1000} onBlur={e => onChange(item, { fadeInMs: Math.round(Number(e.target.value) * 1000) })} /></Field><Field label="Fade out (seconds)"><input type="number" min="0" max="30" step="0.1" defaultValue={(item.fadeOutMs || 0) / 1000} onBlur={e => onChange(item, { fadeOutMs: Math.round(Number(e.target.value) * 1000) })} /></Field><Field label="Volunteer notes"><input defaultValue={item.notes || ""} placeholder="Shown during playback" onBlur={e => onChange(item, { notes: e.target.value })} /></Field></div><div className="advanced-checks"><label className="check-line"><input type="checkbox" checked={item.allowSkip} onChange={e => onChange(item, { allowSkip: e.target.checked })} /> Allow volunteers to skip this cue</label><label className="check-line"><input type="checkbox" defaultChecked={item.normalizeAudio} onChange={e => onChange(item, { normalizeAudio: e.target.checked })} /> Normalize audio when a processed derivative is available</label></div></details>}
    </div><button className="delete-button" onClick={() => onRemove(item.id)} title="Remove item">×</button>
  </article>;
}

function LessonRunSheet({ lesson }: { lesson: Lesson }) {
  const items = [...lesson.items].sort((a, b) => a.position - b.position);
  const rows = items.map((item, index) => {
    const elapsed = items.slice(0, index).filter(previous => previous.role === "lesson").reduce((sum, previous) => sum + cuePlannedDurationMs(previous), 0);
    const duration = cuePlannedDurationMs(item);
    const planned = lesson.designatedStartAt && item.role === "lesson"
      ? new Date(new Date(lesson.designatedStartAt).getTime() + elapsed).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })
      : roleName(item.role);
    return { item, index, duration, planned };
  });
  return <section className="run-sheet">
    <header><div><span>LESSONCUE RUN SHEET</span><h1>{lesson.title}</h1><p>{lesson.className} · {formatDate(lesson.date)}</p></div><div><strong>{lesson.designatedStartAt ? new Date(lesson.designatedStartAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) : "No fixed start"}</strong><small>{formatFriendlyDuration(lessonPlannedDurationMs(lesson))} estimated</small></div></header>
    {lesson.substituteNotes && <aside><strong>Substitute / teacher instructions</strong><p>{lesson.substituteNotes}</p></aside>}
    <table><thead><tr><th>#</th><th>Planned</th><th>Cue</th><th>Duration</th><th>Operator notes</th></tr></thead><tbody>{rows.map(({ item, index, duration, planned }) => <tr key={item.id} className={item.flexibleTime ? "flexible" : ""}><td>{index + 1}</td><td>{planned}</td><td><strong>{item.title}</strong><small>{item.mediaFileName || item.type}{item.flexibleTime ? " · FLEXIBLE" : ""}</small></td><td>{duration ? formatFriendlyDuration(duration) : "Unknown"}</td><td>{item.notes || "—"}</td></tr>)}</tbody></table>
    <footer><span>Estimated finish: {lesson.designatedStartAt ? new Date(new Date(lesson.designatedStartAt).getTime() + lessonPlannedDurationMs(lesson)).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" }) : "—"}</span><span>Printed {new Date().toLocaleString()}</span></footer>
  </section>;
}
...
function ControllerView({ screens, lessons, classes, controllerPinConfigured, requireLocalRoomControllers, localAddress, userRole, refresh, notify }: { screens: Screen[]; lessons: Lesson[]; classes: LessonClass[]; controllerPinConfigured: boolean; requireLocalRoomControllers: boolean; localAddress: string; userRole: string; refresh: () => void; notify: (s: string) => void }) {
  ...
  const liveScreens = screens.filter(screen => !screen.revoked && !screen.signageOnly && (!room || screen.assignedClassId === room.id));
  ...
  const timingItems = [...(timingLesson?.items || [])].filter(item => item.role === "lesson").sort((a, b) => a.position - b.position);
  const reportedIndex = reportedItem ? timingItems.findIndex(item => item.id === reportedItem.id) : -1;
  const currentRemainingMs = reportedIndex >= 0
    ? Math.max(0, cuePlannedDurationMs(timingItems[reportedIndex]) - (selectedScreen?.playbackPositionMs || 0)) + timingItems.slice(reportedIndex + 1).reduce((sum, item) => sum + cuePlannedDurationMs(item), 0)
    : timingItems.reduce((sum, item) => sum + cuePlannedDurationMs(item), 0);
  const estimatedFinish = timingLesson && currentRemainingMs ? new Date(Date.now() + currentRemainingMs) : undefined;
  const scheduledFinish = timingLesson?.designatedStartAt ? new Date(new Date(timingLesson.designatedStartAt).getTime() + lessonPlannedDurationMs(timingLesson)) : undefined;
  const isOverrun = !!estimatedFinish && !!scheduledFinish && estimatedFinish.getTime() > scheduledFinish.getTime() + 60_000;
  const preRollNow = !!timingLesson?.preRollStartsAt && !!timingLesson?.designatedStartAt && Date.now() >= new Date(timingLesson.preRollStartsAt).getTime() && Date.now() < new Date(timingLesson.designatedStartAt).getTime();
  const showMonitor = !!timingLesson?.preRollMonitorUrl && (monitorOpen || preRollNow || reportedItem?.role === "preRoll");
  ...
  const isAdministrator = userRole === "Service Admin" || userRole === "App Admin" ||
  ...
  return <><PageHead eyebrow="PLAYBACK DEVICES" title="Screens" detail="Pair TVs, assign a class, and inspect cache, downloads, codecs, timing, network quality, and recent errors." action={canManage && pin ? <div className="pin-card"><span>PAIRING PIN</span><strong>{pin}</strong></div> : undefined} />
    ...
    <section className="panel"><div className="screen-grid">{active.length ? active.map(s => {
      ...
      return <article className={`screen-card ${expanded === s.id ? "expanded" : ""}`} key={s.id}>
        ...
        <Field label="Assigned class"><select value={s.assignedClassId || ""} disabled={!canManage || s.signageOnly} onChange={e => change(s, e.target.value ? { assignedClassId: e.target.value } : { clearAssignment: true })}><option value="">Not assigned</option>{classes.map(c => <option value={c.id} key={c.id}>{c.name}</option>)}</select></Field>
        <div className="two-fields"><label className="switch-row compact"><input type="checkbox" checked={!!s.signageOnly} disabled={!canManage} onChange={e => change(s, { signageOnly: e.target.checked })} /><span /><div><strong>Signage only</strong><small>Excludes lessons and controller playback.</small></div></label><label className="switch-row compact"><input type="checkbox" checked={!!s.permanentPairing} disabled={!canManage} onChange={e => change(s, { permanentPairing: e.target.checked })} /><span /><div><strong>Permanent pairing</strong><small>Keep this browser screen after inactivity.</small></div></label></div>
        ...
        <div className="card-actions"><button className="button" disabled={!canManage || busy === s.id} onClick={() => void openScreen(s)}>{busy === s.id ? "Opening…" : "Open screen ↗"}</button><button className="button" onClick={() => setExpanded(expanded === s.id ? undefined : s.id)}>{expanded === s.id ? "Hide diagnostics" : "View diagnostics"}</button></div>
...
function TemplatesView({ templates, schedules, lessons, classes, refresh, notify }: {
  ...
  async function updateTemplate(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await api(`/api/v1/lesson-templates/${editingTemplate.id}`, { method: "PUT", body: JSON.stringify({
        ...
        downloadDaysBefore: Number(values.downloadDaysBefore), volumePercent: Number(values.volumePercent),
  ...
  return <>
    ...
    {editingTemplate && <Modal title="Edit template defaults" onClose={() => !busy && setEditingTemplate(undefined)}><form className="stack" onSubmit={updateTemplate}><div className="two-fields"><Field label="Template name"><input name="name" required defaultValue={editingTemplate.name} /></Field><Field label="Default lesson title"><input name="defaultTitle" required defaultValue={editingTemplate.defaultTitle} /></Field></div><Field label="Description"><textarea name="description" rows={3} defaultValue={editingTemplate.description} /></Field><div className="two-fields"><Field label="Default start time"><input name="defaultStartTime" type="time" defaultValue={timeFromMinutes(editingTemplate.defaultStartMinutes)} /></Field><Field label="Pre-roll lead (minutes)"><input name="preRollLeadMinutes" type="number" min="0" max="1440" defaultValue={editingTemplate.preRollLeadMinutes ?? ""} /></Field></div><div className="two-fields"><Field label="Download days before"><input name="downloadDaysBefore" type="number" min="0" max="365" defaultValue={editingTemplate.downloadDaysBefore} /></Field><Field label="Whole-lesson volume"><div className="unit-input"><input name="volumePercent" type="number" min="0" max="150" defaultValue={editingTemplate.volumePercent ?? 100} /><span>%</span></div></Field></div><div className="template-switches"><label className="check-line"><input name="preRollEnabled" type="checkbox" defaultChecked={editingTemplate.preRollEnabled} /> Enable pre-roll</label><label className="check-line"><input name="keepOffline" type="checkbox" defaultChecked={editingTemplate.keepOffline} /> Keep lesson offline</label><label className="check-line"><input name="muted" type="checkbox" defaultChecked={editingTemplate.muted} /> Mute generated lessons</label></div><button className="button primary" disabled={busy}>Save defaults</button></form><form className="template-replace stack" onSubmit={replaceTemplate}><div><strong>Refresh the complete structure</strong><p>Replace playlist order, media, trims, fades, cue markers, role assignments, volume, and timing defaults from a newer lesson. Referenced media is kept permanently for safe reuse.</p></div><Field label="New source lesson"><select name="lessonId" required>{[...lessons].sort((a, b) => b.date.localeCompare(a.date)).map(lesson => <option key={lesson.id} value={lesson.id}>{formatShortDate(lesson.date)} — {lesson.title}</option>)}</select></Field><button className="button" disabled={busy}>Replace from selected lesson</button></form></Modal>}
...
function CalendarView({ lessons }: { lessons: Lesson[] }) {
  ...
  const [view, setView] = useState<"agenda" | "day" | "week" | "month" | "room">("agenda");
  ...
  const weekStart = new Date(focus); weekStart.setDate(focus.getDate() - focus.getDay());
  const weekDates = Array.from({ length: 7 }, (_, index) => calendarDate(new Date(weekStart.getFullYear(), weekStart.getMonth(), weekStart.getDate() + index)));
  ...
  const monthDates = Array.from({ length: 42 }, (_, index) => calendarDate(new Date(gridStart.getFullYear(), gridStart.getMonth(), gridStart.getDate() + index)));
  const rooms = visible.reduce<Record<string, Lesson[]>>((all, lesson) => { (all[lesson.className] ||= []).push(lesson); return all; }, {});
  ...
  const event = (lesson: Lesson, compact = false) => <CalendarLesson key={lesson.id} lesson={lesson} conflict={conflicts.has(lesson.id)} compact={compact} />;
  return <><PageHead eyebrow="SCHEDULE" title="Calendar" detail="Day, week, month, agenda, and room views with estimated run times and overlap warnings." />
    <section className="calendar-toolbar panel"><div className="calendar-view-tabs" role="group" aria-label="Calendar view">{(["agenda", "day", "week", "month", "room"] as const).map(option => <button key={option} className={view === option ? "active" : ""} onClick={() => setView(option)}>{option[0].toUpperCase() + option.slice(1)}</button>)}</div>{view !== "agenda" && view !== "room" && <div className="calendar-navigation"><button onClick={() => moveFocus(-1)} aria-label="Previous period">‹</button><button onClick={() => setFocusDate(dateInputValue())}>Today</button><input type="date" value={focusDate} onChange={event => setFocusDate(event.target.value)} aria-label="Calendar date" /><button onClick={() => moveFocus(1)} aria-label="Next period">›</button></div>}</section>
    {!visible.length && <section className="panel"><Empty title="No lessons scheduled" body="Create a dated lesson from Classes and it will appear here." /></section>}
    {visible.length > 0 && view === "agenda" && <div className="calendar-stack">{Object.entries(grouped).map(([month, entries]) => <section className="panel" key={month}><div className="panel-title"><h2>{new Date(`${month}-15T12:00:00`).toLocaleDateString(undefined, { month: "long", year: "numeric" })}</h2><span className="pill">{entries.length} lessons</span></div><div className="calendar-grid">{entries.map(lesson => event(lesson))}</div></section>)}</div>}
    {visible.length > 0 && view === "day" && <section className="panel calendar-period"><div className="panel-title"><h2>{focus.toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" })}</h2><span className="pill">{visible.filter(item => item.date === focusDate).length} lessons</span></div><div className="calendar-grid">{visible.filter(item => item.date === focusDate).map(lesson => event(lesson))}{!visible.some(item => item.date === focusDate) && <Empty title="No lessons this day" body="Choose another date or create a lesson." />}</div></section>}
    {visible.length > 0 && view === "week" && <section className="calendar-week panel">{weekDates.map(date => <div className={date === dateInputValue() ? "today" : ""} key={date}><header><strong>{new Date(`${date}T12:00:00`).toLocaleDateString(undefined, { weekday: "short" })}</strong><span>{new Date(`${date}T12:00:00`).getDate()}</span></header>{visible.filter(item => item.date === date).map(lesson => event(lesson, true))}</div>)}</section>}
    {visible.length > 0 && view === "month" && <section className="calendar-month panel">{["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(day => <strong className="calendar-weekday" key={day}>{day}</strong>)}{monthDates.map(date => <div className={`${date.slice(0, 7) !== focusDate.slice(0, 7) ? "outside" : ""} ${date === dateInputValue() ? "today" : ""}`} key={date}><span>{new Date(`${date}T12:00:00`).getDate()}</span>{visible.filter(item => item.date === date).map(lesson => event(lesson, true))}</div>)}</section>}
    {visible.length > 0 && view === "room" && <div className="calendar-rooms">{Object.entries(rooms).sort(([a], [b]) => a.localeCompare(b)).map(([room, entries]) => <section className="panel" key={room}><div className="panel-title"><h2>{room}</h2><span className="pill">{entries.length} lessons</span></div><div className="calendar-grid">{entries.map(lesson => event(lesson))}</div></section>)}</div>}
  </>;
}

function CalendarLesson({ lesson, conflict, compact }: { lesson: Lesson; conflict: boolean; compact?: boolean }) {
  const duration = lessonPlannedDurationMs(lesson);
  const finish = lesson.designatedStartAt ? new Date(new Date(lesson.designatedStartAt).getTime() + duration) : undefined;
  return <article className={`calendar-lesson ${conflict ? "conflict" : ""} ${compact ? "compact" : ""}`}>{!compact && <DateBadge date={lesson.date} />}<div><strong>{lesson.title}</strong><small>{lesson.className}{lesson.generatedByScheduleId ? " · Recurring" : ""}</small><div className="calendar-meta"><span>{lesson.designatedStartAt ? `${new Date(lesson.designatedStartAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}${finish ? `–${finish.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}` : ""}` : "Start time not set"}</span>{duration > 0 && <span>{formatFriendlyDuration(duration)}</span>}{conflict && <b title="This estimated time overlaps another scheduled lesson">⚠ Conflict</b>}{!compact && <RoleSummary items={lesson.items} />}</div></div></article>;
}

function SignageView({ signage, media, screens, timeZone, sourceAllowlist, refresh, notify }: {
  ...
  async function refreshWidgets(item: Signage) {
    try {
      await api(`/api/v1/signage/${item.id}/widgets/refresh`, { method: "POST", body: "{}" });
...
function UsersView({ users, currentUsername, currentRole, refresh, notify, canManage, emailConfigured }: { users: User[]; currentUsername: string; currentRole: string; refresh: () => void; notify: (s: string) => void; canManage: boolean; emailConfigured: boolean }) {
  ...
  return <><PageHead eyebrow="ACCESS CONTROL" title="Users" detail="Create accounts, approve requests, send setup invitations, reset passwords, and grant only the permissions each person needs." action={canManage ? <div className="head-actions"><button className="button primary" disabled={!emailConfigured} title={!emailConfigured ? "Configure account email under Settings → Organization & accounts first." : undefined} onClick={() => { setCreationMode("invite"); setShowForm(true); }}>Send setup link</button><button className="button" onClick={() => { setCreationMode("temporary"); setShowForm(true); }}>Create with password</button></div> : undefined} />
    ...
    {showForm && <Modal title={creationMode === "invite" ? "Invite a user" : "Create a local user"} onClose={() => setShowForm(false)}><form className="stack" onSubmit={create}>
      ...
      <Field label="Role"><select name="role"><option>Editor</option><option>Viewer</option><option>App Admin</option>{isServiceAdminRole(currentRole) && <option>Service Admin</option>}</select></Field><PermissionEditor />
    ...
    <section className="panel user-table"><div className="user-row user-head"><span>User</span><span>Role</span><span>Status</span><span>Last sign-in</span><span>Actions</span></div>{users.map(user => { const self = user.username === currentUsername; const protectedServiceAdmin = isServiceAdminRole(user.role) && !isServiceAdminRole(currentRole); const status = accountStatus(user); return <div className={`user-row ${user.disabled ? "paused" : ""}`} key={user.id}><span className="user-name"><b>{initials(user.displayName)}</b><span><strong>{user.displayName}{self ? " (you)" : ""}</strong><small>{user.pendingSetup ? "Username chosen during setup" : `@${user.username}`}{user.email ? ` · ${user.email}` : ""}</small><small>{user.permissions.length} of {permissionOptions.length} permissions{user.customPermissions ? " · custom" : " · role defaults"}</small></span></span><span><i className="pill">{user.role}</i></span><span className={`user-status ${status.className}`}><i />{status.label}</span><span>{user.lastLoginAt ? timeAgo(user.lastLoginAt) : "Never"}</span><span className="user-actions">{canManage && <>{user.pendingApproval && <button className="primary-action" onClick={() => approve(user)} disabled={protectedServiceAdmin}>Approve</button>}{user.pendingSetup && <button onClick={() => resendInvitation(user)} disabled={protectedServiceAdmin}>Resend setup</button>}<button onClick={() => setEditingUser(user)} disabled={protectedServiceAdmin}>Edit</button>{!user.pendingSetup && !self && <button onClick={() => setPasswordUser(user)} disabled={protectedServiceAdmin}>Reset password</button>}<button onClick={() => togglePaused(user)} disabled={self || protectedServiceAdmin || user.pendingSetup} title={self ? "You cannot pause your own account." : undefined}>{user.disabled ? "Reactivate" : "Pause"}</button><button className="danger" onClick={() => remove(user)} disabled={self || protectedServiceAdmin} title={self ? "You cannot delete your own account." : undefined}>Delete</button></>}</span></div>; })}</section>
  </>;
}
...
function RegistrationSettingsPanel({ bootstrap, notify, refresh, canServiceSettings }: { bootstrap: Bootstrap; notify: (message: string) => void; refresh: () => void; canServiceSettings: boolean }) {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await api<{ emailConfigured: boolean }>(canServiceSettings ? "/api/v1/registration/settings" : "/api/v1/registration/mode", {
        method: "PUT",
        body: JSON.stringify(canServiceSettings ? { ...settings, apiKey } : { mode: settings.mode }),
      });
      setSettings(current => ({ ...current, emailConfigured: result.emailConfigured }));
      setApiKey(""); refresh(); notify(canServiceSettings ? "Registration and account email settings saved." : "Registration mode saved.");
  ...
  return <section className="panel wide-settings account-settings">
    ...
    <form className="stack" onSubmit={save}>
      ...
      {!canServiceSettings && <div className="alert">{settings.emailConfigured ? "Account email is configured by a Service Admin, so self-service registration modes are available." : "A Service Admin must configure account email before approval, code, or open registration can be enabled."}</div>}
      <button className="button primary" disabled={busy}>{busy ? "Saving…" : canServiceSettings ? "Save account settings" : "Save registration mode"}</button>
    </form>
    {canServiceSettings && <form className="settings-subsection email-test-form" onSubmit={sendTestEmail}>
...
function Settings({ bootstrap, backups, audit, refresh, notify, canAppSettings, canServiceSettings, canBackups, canUpdates }: { bootstrap: Bootstrap; backups: Backup[]; audit: Audit[]; refresh: () => void; notify: (s: string) => void; canAppSettings: boolean; canServiceSettings: boolean; canBackups: boolean; canUpdates: boolean }) {
  ...
  const loadRecycleBin = () => canManageApp ? api<RecycleItem[]>("/api/v1/recycle-bin").then(setRecycleItems).catch(() => undefined) : Promise.resolve();
  useEffect(() => { if (canManageApp) void api<RecycleItem[]>("/api/v1/recycle-bin").then(setRecycleItems).catch(() => undefined); }, [canManageApp]);
  async function saveOrganization(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const values = Object.fromEntries(form); try { await api("/api/v1/organization", { method: "PUT", body: JSON.stringify({ ...values, defaultLessonDurationMinutes: Number(values.defaultLessonDurationMinutes), defaultRetentionDays: Number(values.defaultRetentionDays), requireLocalRoomControllers: form.get("requireLocalRoomControllers") === "on", signageSourceAllowlist: String(values.signageSourceAllowlist || "").split(/[\s,]+/).map(value => value.trim()).filter(Boolean) }) }); refresh(); notify("Organization settings saved."); } catch (e) { notify(errorText(e)); } }
  ...
  return <><PageHead eyebrow="SERVER" title="Settings" detail="Updates, appearance, storage, connectivity, recovery, and local server operations." />
    <nav className="settings-tabs" aria-label="Settings sections">{canUpdates && <button className={settingsSection === "system" ? "active" : ""} onClick={() => setSettingsSection("system")}><span>↻</span><strong>System</strong><small>Updates</small></button>}{canManageApp && <button className={settingsSection === "accounts" ? "active" : ""} onClick={() => setSettingsSection("accounts")}><span>♙</span><strong>{canServiceSettings ? "Organization & accounts" : "Registration"}</strong><small>{canServiceSettings ? "Appearance, registration, email" : "Modes and registration codes"}</small></button>}{canManageApp && <button className={settingsSection === "media" ? "active" : ""} onClick={() => setSettingsSection("media")}><span>▶</span><strong>Media & storage</strong><small>{canServiceSettings ? "Folders, capacity, playback" : "Approved folders and tags"}</small></button>}{canManageApp && <button className={settingsSection === "connections" ? "active" : ""} onClick={() => setSettingsSection("connections")}><span>⌁</span><strong>Connections & pairing</strong><small>{canServiceSettings ? "Address, pairing, remote access" : "Pairing and controller PINs"}</small></button>}{(canManageApp || canBackups) && <button className={settingsSection === "data" ? "active" : ""} onClick={() => setSettingsSection("data")}><span>▤</span><strong>Data & recovery</strong><small>{canServiceSettings ? "Backups, recycle bin, activity" : "Recycle bin and activity"}</small></button>}</nav>
    <div className="settings-page" data-section={settingsSection}>
    {canManageApp && <div className="settings-grid account-settings-grid settings-panel settings-accounts"><RegistrationSettingsPanel bootstrap={bootstrap} notify={notify} refresh={refresh} canServiceSettings={canServiceSettings} /></div>}
...
function TimelineEditor({ media, item, onSave }: { media?: Media; item: PlaylistItem; onSave: (changes: Record<string, unknown>) => void | Promise<void> }) {
  ...
  const [activeControl, setActiveControl] = useState<"in" | "out" | "fade-in" | "fade-out">("in");
  ...
  function seek(value: number, edge: "start" | "end") {
    ...
    const playablePosition = edge === "end" ? Math.max(nextStart, Math.min(duration - .001, nextEnd - .001)) : nextStart;
    setActiveControl(edge === "start" ? "in" : "out");
    setCursor(exactEdge);
    setVisualFadeOpacity(edge === "start" && fadeIn > 0 || edge === "end" && fadeOut > 0 ? 1 : 0);
    if (player.current) { player.current.pause(); player.current.currentTime = playablePosition; }
  }
  function changeFade(value: number, edge: "in" | "out") {
    const next = Math.max(0, Math.min(end - start, Math.round(value * 10) / 10));
    if (edge === "in") setFadeIn(next); else setFadeOut(next);
    const previewPosition = edge === "in" ? start + next / 2 : end - next / 2;
    setActiveControl(edge === "in" ? "fade-in" : "fade-out");
    setCursor(previewPosition);
    setVisualFadeOpacity(next > 0 ? .5 : 0);
    if (player.current) { player.current.pause(); player.current.currentTime = Math.max(0, Math.min(duration - .001, previewPosition)); }
  }
  function pointerValue(clientX: number) {
    const bounds = timeline.current?.getBoundingClientRect();
    return bounds ? Math.max(0, Math.min(duration, (clientX - bounds.left) / bounds.width * duration)) : 0;
  }
  function beginTrim(event: React.PointerEvent<HTMLButtonElement>, edge: "start" | "end") {
    event.preventDefault();
    event.currentTarget.setPointerCapture(event.pointerId);
    seek(pointerValue(event.clientX), edge);
  }
  function dragTrim(event: React.PointerEvent<HTMLButtonElement>, edge: "start" | "end") {
    if (event.currentTarget.hasPointerCapture(event.pointerId)) seek(pointerValue(event.clientX), edge);
  }
  function nudgeTrim(event: React.KeyboardEvent<HTMLButtonElement>, edge: "start" | "end") {
    if (event.key !== "ArrowLeft" && event.key !== "ArrowRight") return;
    event.preventDefault();
    seek((edge === "start" ? start : end) + (event.key === "ArrowRight" ? .04 : -.04), edge);
  }
  function beginFade(event: React.PointerEvent<HTMLButtonElement>, edge: "in" | "out") {
    event.preventDefault();
    event.currentTarget.setPointerCapture(event.pointerId);
    changeFade(edge === "in" ? pointerValue(event.clientX) - start : end - pointerValue(event.clientX), edge);
  }
  function dragFade(event: React.PointerEvent<HTMLButtonElement>, edge: "in" | "out") {
    if (event.currentTarget.hasPointerCapture(event.pointerId))
      changeFade(edge === "in" ? pointerValue(event.clientX) - start : end - pointerValue(event.clientX), edge);
  }
  function nudgeFade(event: React.KeyboardEvent<HTMLButtonElement>, edge: "in" | "out") {
    if (event.key !== "ArrowLeft" && event.key !== "ArrowRight") return;
    event.preventDefault();
    const direction = event.key === "ArrowRight" ? .1 : -.1;
    changeFade((edge === "in" ? fadeIn : fadeOut) + (edge === "in" ? direction : -direction), edge);
  }
  function updatePreview(element: HTMLMediaElement) {
    ...
    element.volume = item.muted ? 0 : Math.max(0, Math.min(1, item.volumePercent / 100 * fade));
    element.playbackRate = Math.max(.25, Math.min(4, (item.playbackRatePercent || 100) / 100));
    const transition = item.transitionStyle === "fade-black" ? (item.transitionDurationMs || 0) / 1000 : 0;
    const transitionFade = transition ? Math.min(intoSelection / transition, remaining / transition, 1) : 1;
  ...
  return <section className="timeline-editor">
    <div className={`timeline-player editing-${activeControl}`} style={{ backgroundColor: item.fitMode === "letterbox" ? "#000000" : item.backgroundColor || "#000000" }}>{media.contentType.startsWith("video/") ? <><video style={cuePreviewStyle(item)} ref={player as React.RefObject<HTMLVideoElement>} src={source} controls playsInline onLoadedMetadata={e => { e.currentTarget.currentTime = start; updatePreview(e.currentTarget); }} onTimeUpdate={e => updatePreview(e.currentTarget)} /><span className="visual-fade-overlay" style={{ opacity: visualFadeOpacity }} /></> : <audio ref={player as React.RefObject<HTMLAudioElement>} src={source} controls onLoadedMetadata={e => { e.currentTarget.currentTime = start; }} onTimeUpdate={e => updatePreview(e.currentTarget)} />}<span className="timeline-preview-label">Previewing {activeControl === "in" ? "trim in" : activeControl === "out" ? "trim out" : activeControl.replace("-", " ")} · {formatPreciseTime(cursor)}</span></div>
    <div ref={timeline} className="timeline-art" aria-label="Media filmstrip, waveform, selected playback area, fade regions, cue markers, and draggable trim handles">{media.filmstripUrl && <img src={media.filmstripUrl} alt="Video filmstrip" />}{media.waveformUrl && <img className="waveform" src={media.waveformUrl} alt="Audio waveform" />}<i className="trim-before" style={{ width: `${startPercent}%` }} /><i className="trim-after" style={{ left: `${endPercent}%` }} /><span className="selection" style={{ left: `${startPercent}%`, width: `${Math.max(0, endPercent - startPercent)}%` }} />{fadeIn > 0 && <span className={`fade-zone fade-in ${activeControl === "fade-in" ? "active" : ""}`} style={{ left: `${startPercent}%`, width: `${Math.min(endPercent - startPercent, fadeIn / duration * 100)}%` }}>FADE IN</span>}{fadeOut > 0 && <span className={`fade-zone fade-out ${activeControl === "fade-out" ? "active" : ""}`} style={{ left: `${Math.max(startPercent, endPercent - fadeOut / duration * 100)}%`, width: `${Math.min(endPercent - startPercent, fadeOut / duration * 100)}%` }}>FADE OUT</span>}<button type="button" className={`trim-handle trim-start ${activeControl === "in" ? "active" : ""}`} style={{ left: `${startPercent}%` }} aria-label={`Trim in at ${formatPreciseTime(start)}. Drag to adjust.`} onPointerDown={event => beginTrim(event, "start")} onPointerMove={event => dragTrim(event, "start")} onKeyDown={event => nudgeTrim(event, "start")}><span>IN</span></button><button type="button" className={`trim-handle trim-end ${activeControl === "out" ? "active" : ""}`} style={{ left: `${endPercent}%` }} aria-label={`Trim out at ${formatPreciseTime(end)}. Drag to adjust.`} onPointerDown={event => beginTrim(event, "end")} onPointerMove={event => dragTrim(event, "end")} onKeyDown={event => nudgeTrim(event, "end")}><span>OUT</span></button><button type="button" className={`fade-handle fade-start ${activeControl === "fade-in" ? "active" : ""}`} style={{ left: `${startPercent + fadeIn / duration * 100}%` }} aria-label={`Fade in over ${fadeIn.toFixed(1)} seconds. Drag to adjust.`} onPointerDown={event => beginFade(event, "in")} onPointerMove={event => dragFade(event, "in")} onKeyDown={event => nudgeFade(event, "in")}><span /></button><button type="button" className={`fade-handle fade-end ${activeControl === "fade-out" ? "active" : ""}`} style={{ left: `${endPercent - fadeOut / duration * 100}%` }} aria-label={`Fade out over ${fadeOut.toFixed(1)} seconds. Drag to adjust.`} onPointerDown={event => beginFade(event, "out")} onPointerMove={event => dragFade(event, "out")} onKeyDown={event => nudgeFade(event, "out")}><span /></button>{markers.map((marker, index) => <button type="button" className="timeline-marker" style={{ left: `${Math.min(100, marker.positionMs / 1000 / duration * 100)}%` }} title={`${marker.name} · ${formatPreciseTime(marker.positionMs / 1000)}`} aria-label={`Jump preview to ${marker.name}`} onClick={() => jumpToMarker(marker)} key={`${marker.positionMs}-${index}`}><span /></button>)}</div>
    ...
    <div className="timeline-fades"><Field label={`Fade in · ${fadeIn.toFixed(1)}s`}><input type="range" min="0" max={Math.min(30, end - start)} step="0.1" value={fadeIn} onChange={e => changeFade(Number(e.target.value), "in")} /></Field><Field label={`Fade out · ${fadeOut.toFixed(1)}s`}><input type="range" min="0" max={Math.min(30, end - start)} step="0.1" value={fadeOut} onChange={e => changeFade(Number(e.target.value), "out")} /></Field></div>
    ...
    <small>Drag the IN and OUT handles directly on the filmstrip or use the sliders. The preview follows the edge you are adjusting; arrow keys nudge a focused handle by one 0.04-second frame step. Dark shading will not play. Blue fade regions fade both audio and picture to or from black and preview at the midpoint while you adjust them.</small>
  </section>;
}
...
function MediaPreview({ media, item }: { media?: Media; item?: PlaylistItem }) {
  ...
  const targetVolume = item?.muted ? 0 : Math.min(1, (item?.volumePercent ?? 100) / 100);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      ...
      element.playbackRate = Math.max(.25, Math.min(4, (item?.playbackRatePercent || 100) / 100));
      const transitionMs = item?.transitionStyle === "fade-black" ? item.transitionDurationMs || 0 : 0;
      const transitionIn = transitionMs ? Math.min(1, Math.max(0, (current - startMs) / transitionMs)) : 1;
      const transitionOut = transitionMs && actualEnd ? Math.min(1, Math.max(0, (actualEnd - current) / transitionMs)) : 1;
    ...
    return () => window.clearInterval(timer);
  }, [fadeInMs, fadeOutMs, item?.endBehavior, item?.playbackRatePercent, item?.transitionDurationMs, item?.transitionStyle, requestedEnd, startMs, targetVolume]);
  ...
  const stageStyle = item ? { backgroundColor: item.fitMode === "letterbox" ? "#000000" : item.backgroundColor || "#000000" } : undefined;
  const mediaElement = media.contentType.startsWith("video") ? <video style={visualStyle} ref={player as React.RefObject<HTMLVideoElement>} src={source} controls autoPlay playsInline onLoadedMetadata={event => { event.currentTarget.currentTime = startMs / 1000; }} />
    ...
    : media.contentType.startsWith("image") ? <img style={visualStyle} src={source} alt={media.fileName} />
    ...
    : online ? <iframe style={visualStyle} src={frameSource} title={media.fileName} allow="autoplay; fullscreen" />
    : <iframe src={source} title={media.fileName} />;
  return <div className="media-preview"><div className="preview-stage" style={stageStyle}>{mediaElement}{media.contentType.startsWith("video") && item && <span className="visual-fade-overlay" style={{ opacity: visualFadeOpacity }} />}{item?.notes && <div className="preview-notes">{item.notes}</div>}</div>{item && <div className="preview-readout"><span>Position <strong>{formatDuration(positionMs)}</strong></span><span>Trim <strong>{formatDuration(startMs)} → {requestedEnd ? formatDuration(requestedEnd) : "media end"}</strong></span><span>Fades <strong>{(fadeInMs / 1000).toFixed(1)}s in · {(fadeOutMs / 1000).toFixed(1)}s out</strong></span><span>Volume <strong>{item.muted ? "Muted" : `${item.volumePercent}%`}</strong></span><span>Picture <strong>{item.fitMode || "fit"} · {item.rotationDegrees || 0}°</strong></span></div>}{online && <a className="preview-open" href={source} target="_blank" rel="noreferrer">Open original page ↗</a>}</div>;
}

function cuePreviewStyle(item: PlaylistItem): React.CSSProperties {
  const cropLeft = item.cropLeftPercent || 0; const cropRight = item.cropRightPercent || 0;
  const cropTop = item.cropTopPercent || 0; const cropBottom = item.cropBottomPercent || 0;
...
function signageFormPayload(form: FormData) {
  ...
  try { zones = JSON.parse(String(form.get("zonesJson") || "[]")); } catch { zones = []; }
  return {
...
function applySignagePreset(preset: SignageLayoutPreset, existing: SignageZone[]): SignageZone[] {
  const geometry: Record<SignageLayoutPreset, [number, number, number, number][]> = {
    ...
    "header-grid": [[0, 0, 100, 28], [0, 29, 50, 71], [51, 29, 49, 71]], dashboard: [[0, 0, 50, 50], [51, 0, 49, 50], [0, 51, 50, 49], [51, 51, 49, 49]],
  };
  ...
  return Array.from({ length: Math.min(8, target) }, (_, index) => {
    const zone = existing[index] || newSignageZone(index === 0 ? "text" : index === 1 ? "clock" : "text", index === 0 ? "Welcome" : "");
    const [x, y, width, height] = slots[index] || [5 + (index % 3) * 31, 62 + Math.floor(index / 3) * 18, 29, 16];
...
function cuePlannedDurationMs(item: PlaylistItem) {
  const base = item.type === "image"
    ...
    : Math.max(0, (item.endMs ?? item.mediaDurationMs ?? item.durationMs ?? 0) - (item.startMs || 0));
  return Math.round(base * Math.max(1, item.repeatCount || 1) * 100 / Math.max(25, item.playbackRatePercent || 100));
}
function lessonPlannedDurationMs(lesson: Lesson) { return lesson.items.filter(item => item.role === "lesson").reduce((sum, item) => sum + cuePlannedDurationMs(item), 0); }
function formatFriendlyDuration(ms: number) {
  const seconds = Math.max(0, Math.round(ms / 1000));
  const hours = Math.floor(seconds / 3600);
  const minutes = Math.floor((seconds % 3600) / 60);
  const remainder = seconds % 60;
  return [hours ? `${hours}h` : "", minutes ? `${minutes}m` : "", !hours && remainder ? `${remainder}s` : ""].filter(Boolean).join(" ") || "0m";
}
function intervalsOverlap(startA: Date, endA: Date, startB: Date, endB: Date) { return startA < endB && startB < endA; }
function calendarDate(value: Date) { return `${value.getFullYear()}-${String(value.getMonth() + 1).padStart(2, "0")}-${String(value.getDate()).padStart(2, "0")}`; }
function calendarConflictIds(lessons: Lesson[]) {
  const ids = new Set<string>();
  const scheduled = lessons.filter(lesson => lesson.designatedStartAt && lessonPlannedDurationMs(lesson) > 0);
  for (let index = 0; index < scheduled.length; index++) for (let otherIndex = index + 1; otherIndex < scheduled.length; otherIndex++) {
    const first = scheduled[index], second = scheduled[otherIndex];
    if (first.classId !== second.classId) continue;
    const firstStart = new Date(first.designatedStartAt!); const secondStart = new Date(second.designatedStartAt!);
    if (intervalsOverlap(firstStart, new Date(firstStart.getTime() + lessonPlannedDurationMs(first)), secondStart, new Date(secondStart.getTime() + lessonPlannedDurationMs(second)))) { ids.add(first.id); ids.add(second.id); }
  }
  return ids;
}
...
function isServiceAdminRole(role: string) { return role === "Service Admin" || role === "Owner"; }
