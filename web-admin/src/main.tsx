...
function ClassesView({ classes, lessons, media, refresh, notify, canUpload, storage }: { classes: LessonClass[]; lessons: Lesson[]; media: Media[]; refresh: () => void; notify: (s: string) => void; canUpload: boolean; storage?: StorageStatus }) {
  ...
  async function applyLessonBulk(event: FormEvent<HTMLFormElement>) {
    ...
    const values = Object.fromEntries(new FormData(event.currentTarget));
    if (lessonBulkAction === "delete" && !confirm(`Permanently delete ${selectedLessons.length} selected lesson${selectedLessons.length === 1 ? "" : "s"}? Playlist cues will also be removed.`)) return;
    setBulkBusy(true);
    try {
  ...
  return <>
    ...
    {!classes.length ? <section className="panel"><Empty title="Create your first class" body="Classes organize lessons and determine which screens receive them." action={<button className="button primary" onClick={() => setShowClassForm(true)}>Create class</button>} /></section> : <div className="classes-layout">
      ...
      <section className="panel class-detail"><div className="class-title"><div><span className="eyebrow">CLASS</span><h2>{current?.name}</h2><p>{current?.description || "No class description yet."}</p></div>{current && <button className="button" onClick={() => setShowControllerSettings(true)}>Controller link</button>}</div>
...
function MediaManagerModal({ media, impact, lessons, busy, conversionLessonId, slideSeconds, onClose,
  ...
  return <Modal title={`Manage: ${media.fileName}`} onClose={onClose}><div className="media-manager">
    ...
    {media.contentType.startsWith("video/") && <section className={`compatibility-card ${media.compatibilityStatus}`}><div><span>TV PLAYBACK COMPATIBILITY</span><strong>{media.compatibilityStatus === "ready" ? "Universal H.264/AAC copy ready" : media.compatibilityStatus === "native" ? "Original is TV-compatible" : media.compatibilityStatus === "converting" ? "Creating a compatible copy…" : media.compatibilityStatus === "failed" ? "Compatibility conversion failed" : "Compatibility check pending"}</strong><p>{media.compatibilityError || (media.compatibilityStatus === "ready" ? `LessonCue kept the original and serves a ${formatBytes(media.compatibilitySizeBytes || 0)} MP4 copy to TVs and previews.` : "LessonCue checks every upload locally and converts only when the original may not play reliably on Android TV, Fire TV, or Apple TV.")}</p></div>{media.compatibilityTranscodedAt && <small>{new Date(media.compatibilityTranscodedAt).toLocaleString()}</small>}</section>}
