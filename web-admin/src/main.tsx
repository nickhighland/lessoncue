...
type CloudflareTunnelStatus = { enabled: boolean; publicHostname?: string; publicUrl?: string; originUrl: string; supported: boolean; pending: boolean; credentialConfigured: boolean; serviceInstalled: boolean; connected: boolean; activeConnections: number; cloudflaredVersion?: string; appliedAt?: string; error?: string };
...
type MediaTranscode = { id: string; profile: "h264-720" | "h264-480"; status: "pending" | "converting" | "ready" | "failed"; sizeBytes: number; width?: number; height?: number; videoBitrateKbps: number; sourceVersion: number; error?: string; queuedAt: string; startedAt?: string; completedAt?: string };
type Media = { id: string; fileName: string; contentType: string; sizeBytes: number; durationMs?: number; downloadUrl: string; playbackUrl?: string; thumbnailUrl?: string; filmstripUrl?: string; waveformUrl?: string; processingStatus: string; processingError?: string; videoCodec?: string; audioCodec?: string; width?: number; height?: number; compatibilityStatus: string; compatibilityError?: string; compatibilityTranscodedAt?: string; compatibilitySizeBytes?: number; transcodes: MediaTranscode[]; sourceKind: string; sourceUrl?: string; linkKind?: string; offlineEligible: boolean; storagePolicy: "lesson" | "persistent"; originLessonId?: string; deleteAfter?: string; retentionDateIsManual: boolean; folder: string; tagsCsv: string; version: number; replacedAt?: string; conversionStatus: string; conversionError?: string; convertedSlidesJson: string; convertedAt?: string };
...
function classControllerUrl(item: LessonClass, lessonId = "") {
  const origin = item.controllerHostname ? `https://${item.controllerHostname}` : location.origin;
  ...
  return <><a className="skip-link" href="#main-content">Skip to main content</a><div className={`app-shell ${view === "controller" ? "controller-mode" : ""}`}>
    ...
    <main className="content" id="main-content" tabIndex={-1}>
      ...
      {loading && !bootstrap ? <div className="loading">Loading local data…</div> : <>
        ...
        {view === "controller" && bootstrap && <ControllerView screens={screens} lessons={lessons} classes={classes} controllerPinConfigured={bootstrap.controllerPinConfigured} refresh={refresh} notify={setNotice} />}
        {view === "classes" && <ClassesView classes={classes} lessons={lessons} media={media} taxonomy={bootstrap?.mediaTaxonomy || { folders: [], tags: [] }} refresh={refresh} notify={setNotice} canUpload={canUpload} storage={bootstrap?.storage} />}
...
function ClassesView({ classes, lessons, media, taxonomy, refresh, notify, canUpload, storage }: { classes: LessonClass[]; lessons: Lesson[]; media: Media[]; taxonomy: MediaTaxonomy; refresh: () => void; notify: (s: string) => void; canUpload: boolean; storage?: StorageStatus }) {
...
function LessonEditor({ lesson, media, taxonomy, onBack, refresh, notify, canUpload, storage }: { lesson: Lesson; media: Media[]; taxonomy: MediaTaxonomy; onBack: () => void; refresh: () => void; notify: (s: string) => void; canUpload: boolean; storage?: StorageStatus }) {
  ...
  async function uploadAndAdd(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      notify(persistent ? `${files.length} file${files.length === 1 ? "" : "s"} uploaded permanently and added to the lesson.` : `${files.length} file${files.length === 1 ? "" : "s"} added. ${files.length === 1 ? "It" : "They"} will be deleted four weeks after ${formatDate(lesson.date)}.`);
    } catch (e) { notify(errorText(e)); }
  ...
  async function addOnline(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const asset = await api<Media>("/api/v1/media/link", { method: "POST", body: JSON.stringify({ url: form.get("url"), title: form.get("title") || null, download, persistent, lessonId: persistent ? null : lesson.id, folder: form.get("folder"), tagsCsv: formTags(form) }) });
  ...
  return <>
    ...
    {showAdd && canUpload && <Modal title="Add media to the lesson" onClose={() => !uploading && setShowAdd(false)}><div className="add-media-options">
      <section><h3>Upload from this computer</h3><p>Select one file or add a complete group in order without leaving this lesson. {storage && `${formatBytes(storage.remainingBytes)} remains available.`}</p><form className="stack" onSubmit={uploadAndAdd}><Field label="Media files" hint="When several files are selected, each filename becomes its cue title."><input name="files" type="file" multiple accept="video/*,audio/*,image/*" required disabled={uploading} /></Field><RetentionChoices lessonDate={lesson.date} /><TaxonomyFields taxonomy={taxonomy} /><div className="two-fields"><Field label="Playlist role"><select name="role"><option value="lesson">Main lesson</option><option value="preRoll">Pre-roll loop</option><option value="countdown">Countdown video (one file)</option></select></Field><Field label="Display title" hint="Used only when one file is selected."><input name="title" placeholder="Use filename" /></Field></div><button type="submit" className="button primary" disabled={uploading}>{uploading ? `Uploading ${uploadProgress}%` : "Upload and add"}</button></form></section>
...
function ControllerView({ screens, lessons, classes, controllerPinConfigured, refresh, notify }: { screens: Screen[]; lessons: Lesson[]; classes: LessonClass[]; controllerPinConfigured: boolean; refresh: () => void; notify: (s: string) => void }) {
  ...
  async function command(action: string, extras: Record<string, unknown> = {}) {
    ...
    try {
      ...
      await api(`/api/v1/screens/${screenId}/control`, { method: "POST", headers: controllerHeaders, body: JSON.stringify({ action, ...extras }) });
  ...
  const controllerStyle = room ? { "--gold": room.controllerColor, "--green": room.controllerColor } as CSSProperties : undefined;
  return <div className="controller-page" style={controllerStyle}><PageHead eyebrow={temporarySession ? "TEMPORARY CONTROL" : room ? "CLASSROOM CONTROL" : "LIVE CONTROL"} title={room ? room.name : "Universal controller"} detail={temporarySession ? `Restricted link · expires ${new Date(temporarySession.expiresAt).toLocaleString()}` : room ? `This controller is restricted to ${room.name} screens and lessons.` : "Choose any paired screen, then run its assigned lesson from this phone."} action={<span className={`controller-connection ${selectedScreen?.online ? "online" : ""}`}><i />{selectedScreen?.online ? "Screen online" : "Screen offline"}</span>} />
    ...
    </div><section className="controller-install"><div className="brand-mark">LC</div><div><strong>Save this controller as an app</strong><p>On iPhone or iPad, use Share → Add to Home Screen. On Android, open the browser menu and choose Install app or Add to Home screen.</p><small>{temporarySession ? `${location.origin}/session/${sessionToken}` : room ? classControllerUrl(room) : `${location.origin}/universalremote`}</small></div></section>
  </div>;
}
...
function Settings({ bootstrap, backups, audit, refresh, notify, canSettings, canBackups, canUpdates }: { bootstrap: Bootstrap; backups: Backup[]; audit: Audit[]; refresh: () => void; notify: (s: string) => void; canSettings: boolean; canBackups: boolean; canUpdates: boolean }) {
  ...
  async function saveAdaptiveTranscoding(event: FormEvent<HTMLFormElement>) { event.preventDefault(); try { await api("/api/v1/organization", { method: "PUT", body: JSON.stringify({ ...o, adaptiveTranscodingEnabled: adaptiveTranscoding, transcodeLeadDays: Number(transcodeLeadDays) }) }); refresh(); notify(adaptiveTranscoding ? "Adaptive TV copies will be prepared before assigned lessons." : "Automatic adaptive copy generation is off; existing copies remain available."); } catch (e) { notify(errorText(e)); } }
  ...
  return <><PageHead eyebrow="SERVER" title="Settings" detail="Updates, appearance, storage, connectivity, recovery, and local server operations." />
    ...
    <div className="settings-grid"><section className="panel wide-settings update-settings"><div className="settings-heading"><div><span className="settings-kicker">SYSTEM MAINTENANCE</span><h2>Software updates</h2><p className="settings-copy">LessonCue checks once each day and alerts administrators when a newer release is available.</p></div><span className={`update-state ${bootstrap.update.updateAvailable ? "available" : "current"}`}>{bootstrap.update.updateAvailable ? "Update available" : "Up to date"}</span></div><div className="storage-facts"><Definition label="Installed version" value={bootstrap.update.currentVersion} /><Definition label="Latest version" value={bootstrap.update.latestVersion || "Not checked yet"} /><Definition label="Last checked" value={bootstrap.update.lastCheckedAt ? timeAgo(bootstrap.update.lastCheckedAt) : "Not checked yet"} /></div>{bootstrap.update.error && <div className="alert error">{bootstrap.update.error}</div>}<div className="head-actions"><button className="button" onClick={checkUpdates} disabled={!canUpdates || checking}>{checking ? "Checking…" : "Check now"}</button>{canUpdates && bootstrap.update.updateAvailable && bootstrap.update.automaticInstallSupported && <button className="button primary" onClick={installUpdate} disabled={installing}>{installing ? "Installing…" : `Install ${bootstrap.update.latestVersion}`}</button>}{bootstrap.update.releaseUrl && <a className="button" href={bootstrap.update.releaseUrl} target="_blank" rel="noreferrer">Release notes</a>}</div>{!canUpdates && <p className="settings-copy">An account with software-update permission controls update checks and installation.</p>}{!bootstrap.update.automaticInstallSupported && <p className="settings-copy">Run the current release installer once from SSH to enable automatic updates on this server.</p>}</section>
      <section className="panel wide-settings"><h2>Organization & appearance</h2><p className="settings-copy">Manage organization defaults and every interface color together.</p><form className="stack" onSubmit={saveOrganization}><div className="two-fields"><Field label="Organization"><input name="name" defaultValue={o.name} disabled={!canManage} required /></Field><Field label="Site"><input name="siteName" defaultValue={o.siteName} disabled={!canManage} required /></Field></div><div className="two-fields"><Field label="Time zone"><input name="timeZone" defaultValue={o.timeZone} disabled={!canManage} required /></Field><Field label="Week starts"><select name="weekStartsOn" defaultValue={o.weekStartsOn} disabled={!canManage}><option>Sunday</option><option>Monday</option></select></Field></div><Field label="Welcome message"><input name="welcomeMessage" defaultValue={o.welcomeMessage} disabled={!canManage} /></Field><div className="two-fields"><Field label="Default lesson minutes"><input name="defaultLessonDurationMinutes" type="number" min="5" max="480" defaultValue={o.defaultLessonDurationMinutes} disabled={!canManage} /></Field><Field label="Archive retention days"><input name="defaultRetentionDays" type="number" min="1" max="3650" defaultValue={o.defaultRetentionDays} disabled={!canManage} /></Field></div><div className="settings-subsection"><h3>Interface colors</h3><p>Choose the navigation background, general accent, navigation text, and selected-tab colors in one place.</p><div className="color-fields"><Field label="Navigation background"><input name="primaryColor" type="color" defaultValue={o.primaryColor} disabled={!canManage} /></Field><Field label="Accent color"><input name="accentColor" type="color" defaultValue={o.accentColor} disabled={!canManage} /></Field><Field label="Navigation text"><input name="navigationTextColor" type="color" defaultValue={o.navigationTextColor} disabled={!canManage} /></Field><Field label="Selected navigation tab"><input name="selectedTabColor" type="color" defaultValue={o.selectedTabColor} disabled={!canManage} /></Field></div></div>{canManage && <button className="button primary">Save organization & appearance</button>}</form></section>
      ...
      <section className="panel"><h2>Adaptive TV playback</h2><p className="settings-copy">LessonCue can prepare reusable 720p and 480p H.264 copies locally, then choose the best available profile for each paired screen. Copies count toward the storage allocation above.</p>{canManage ? <form className="stack" onSubmit={saveAdaptiveTranscoding}><label className="check-row"><input type="checkbox" checked={adaptiveTranscoding} onChange={e => setAdaptiveTranscoding(e.target.checked)} /> Prepare adaptive copies automatically</label><Field label="Prepare before lesson" hint="Assigned screens determine which profile is generated. The universal 1080p copy remains a safe fallback while processing."><div className="number-suffix"><input type="number" min="1" max="30" step="1" value={transcodeLeadDays} onChange={e => setTranscodeLeadDays(e.target.value)} disabled={!adaptiveTranscoding} required /><span>days</span></div></Field><button className="button primary">Save playback profiles</button></form> : <><Definition label="Automatic preparation" value={o.adaptiveTranscodingEnabled ? "On" : "Off"} /><Definition label="Lead time" value={`${o.transcodeLeadDays} days`} /></>}</section>
