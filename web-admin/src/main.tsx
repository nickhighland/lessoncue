...
function Shell({ view, setView, username, currentUsername, role, onLogout, notice, setNotice }: {
  ...
  const nav: [View, string, string][] = [["dashboard", "⌂", "Dashboard"], ["controller", "⌁", "Controller"], ["classes", "▤", "Classes"], ["templates", "↻", "Templates"], ["calendar", "□", "Calendar"], ["media", "▶", "Media Library"], ["screens", "▣", "Screens"], ["signage", "◇", "Signage"], ["users", "♙", "Users"], ["settings", "⚙", "Settings"]];
  ...
  return <>
    ...
    <div className="editor-grid">
      ...
      <section className="panel playlist-panel"><div className="panel-heading"><div><h2>Playback sequence</h2><p>Pre-roll loops, countdown runs once, then lesson media plays in order.</p></div><span className="pill">{items.length} items</span></div>
        ...
        {items.length ? <div className="playlist">{items.map((item, index) => <PlaylistCueRow key={item.id} item={item} index={index} total={items.length} onMove={move} onChange={changeItem} onTimeline={() => setPreviewItem(item)} onRemove={removeItem} />)}</div> : <Empty title="This playlist is empty" body="Add videos, audio, or images from your local media library." action={<button className="button primary" onClick={() => setShowAdd(true)}>Add media</button>} />}
      </section>
    </div>
  </>;
}

function PlaylistCueRow({ item, index, total, onMove, onChange, onTimeline, onRemove }: {
  ...
  onTimeline: () => void; onRemove: (id: string) => void | Promise<void>;
}) {
  return <article className={`playlist-item ${item.role}`}>
