...
function MediaView({ media, lessons, refresh, notify, canUpload, storage }: { media: Media[]; lessons: Lesson[]; refresh: () => void; notify: (s: string) => void; canUpload: boolean; storage?: StorageStatus }) {
  ...
  return <><PageHead eyebrow="LOCAL STORAGE" title="Media library" detail="Files stay on this server. Lesson media expires automatically; reusable media can be kept permanently." action={canUpload ? <div className="head-actions"><button className="button" onClick={() => setShowLink(true)}>Add link</button><button className="button primary" onClick={() => setShowUpload(true)}>Upload media</button></div> : undefined} />
    ...
    {manageMedia && <Modal title={`Manage: ${manageMedia.fileName}`} onClose={() => !manageBusy && setManageMedia(undefined)}><div className="media-manager"><div className="media-manager-summary"><div><span>CURRENT VERSION</span><strong>v{manageMedia.version}</strong></div><div><span>LESSON USES</span><strong>{mediaImpact?.lessons.reduce((sum, lesson) => sum + lesson.itemCount, 0) ?? "…"}</strong></div><div><span>SIGNAGE USES</span><strong>{mediaImpact?.signage.length ?? "…"}</strong></div></div><div className="head-actions"><button className="button" onClick={() => { setOrganizeTargets([manageMedia]); setManageMedia(undefined); }}>Rename, folder & tags</button>{manageMedia.sourceKind !== "link" && <button className="button" onClick={reprocessMedia} disabled={manageBusy}>Reprocess metadata</button>}</div>{manageMedia.sourceKind !== "link" && <form className="replace-media" onSubmit={replaceMedia}><Field label="Replace current file" hint="Every lesson and sign keeps using this media ID. The current file is archived as a restorable version."><input name="file" type="file" accept="video/*,audio/*,image/*,.pdf,.pptx" required disabled={manageBusy} /></Field><button className="button primary" disabled={manageBusy}>{manageBusy ? "Working…" : "Preview impact and replace"}</button></form>}<section className="impact-list"><h3>Current impact</h3>{mediaImpact?.lessons.map(lesson => <div key={lesson.id}><span>{formatDate(lesson.date)} · {lesson.title}</span><strong>{lesson.itemCount} cue{lesson.itemCount === 1 ? "" : "s"}</strong></div>)}{mediaImpact?.signage.map(sign => <div key={sign.id}><span>{sign.name}</span><strong>{sign.enabled ? "Active sign" : "Inactive sign"}</strong></div>)}{mediaImpact && !mediaImpact.lessons.length && !mediaImpact.signage.length && <p className="settings-copy">This media is not currently used by a lesson or sign.</p>}</section><section className="version-list"><h3>Previous versions</h3>{mediaImpact?.versions.map(version => <div key={version.id}><span><strong>v{version.versionNumber} · {version.fileName}</strong><small>{formatBytes(version.sizeBytes)} · archived {new Date(version.archivedAt).toLocaleString()} by {version.archivedBy}</small></span><div><a className="button" href={version.downloadUrl}>Download</a><button className="button" onClick={() => restoreMediaVersion(version)} disabled={manageBusy}>Restore</button></div></div>)}{mediaImpact && !mediaImpact.versions.length && <p className="settings-copy">No previous versions yet. The first replacement will archive the current file here.</p>}</section></div></Modal>}
    {showUpload && <Modal title="Upload media" onClose={() => !uploading && setShowUpload(false)}><form className="stack" onSubmit={upload}><Field label="Files"><input name="files" type="file" multiple accept="video/*,audio/*,image/*,.pdf,.pptx" required disabled={uploading} /></Field><div className="two-fields"><Field label="Folder"><input name="folder" maxLength={120} placeholder="Unfiled" /></Field><Field label="Tags"><input name="tagsCsv" maxLength={500} placeholder="intro, reusable" /></Field></div><fieldset className="retention-options"><legend>How long should LessonCue keep these files?</legend>{availableLessons.length > 0 && <label><input type="radio" name="storagePolicy" value="lesson" checked={storagePolicy === "lesson"} onChange={() => setStoragePolicy("lesson")} /><span><strong>For a lesson (default)</strong><small>Delete automatically four weeks after the lesson date.</small></span></label>}<label><input type="radio" name="storagePolicy" value="persistent" checked={storagePolicy === "persistent"} onChange={() => setStoragePolicy("persistent")} /><span><strong>Keep permanently</strong><small>Store in the reusable media library until someone deletes it.</small></span></label></fieldset>{storagePolicy === "lesson" && <Field label="Lesson" hint="Reusing the file in a later lesson automatically extends its deletion date."><select name="lessonId" defaultValue={firstUpcoming?.id} required>{availableLessons.map(l => <option value={l.id} key={l.id}>{formatDate(l.date)} — {l.title}</option>)}</select></Field>}{!availableLessons.length && <div className="alert">Create a lesson before uploading temporary lesson media. This upload will be kept permanently.</div>}<button className="button primary" disabled={uploading}>{uploading ? `Uploading ${uploadProgress}%` : "Upload to local server"}</button></form></Modal>}
