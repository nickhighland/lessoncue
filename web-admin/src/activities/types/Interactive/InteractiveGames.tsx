...
export const BuzzerDisplay: React.FC<ActivityComponentProps> = ({ envelope }) => {
  ...
  return (
    <StageShell title={stringOf(config.title, envelope.name || 'Buzzer Battle')} kicker={`⚡ ${stringOf(config.presetLabel, 'BUZZER BATTLE')}`} phase={state.phase} joinCode={state.joinCode} participantCount={state.participantCount}>
...
export const PunchlineDisplay: React.FC<ActivityComponentProps> = ({ envelope }) => {
  ...
  const displayedSubmissions = headToHead && currentMatch
    ...
    : submissions;
  return (
    <StageShell title={stringOf(config.title, envelope.name || 'Punchline')} kicker={`✍ ${stringOf(config.presetLabel, headToHead ? 'HEAD-TO-HEAD CREATIVE' : 'CREATIVE ROUND')}`} phase={state.phase} joinCode={state.joinCode} participantCount={state.participantCount}>
...
export const FakeOutDisplay: React.FC<ActivityComponentProps> = ({ envelope }) => {
  ...
  return (
    <StageShell title={stringOf(config.title, envelope.name || 'Fake Out')} kicker={`🎭 ${stringOf(config.presetLabel, 'TRUTH OR TRAP')}`} phase={state.phase} joinCode={state.joinCode} participantCount={state.participantCount}>
