...
export const PunchlineDisplay: React.FC<ActivityComponentProps> = ({ envelope }) => {
  ...
  return (
    <StageShell title={stringOf(config.title, envelope.name || 'Punchline')} kicker={`✍ ${stringOf(config.presetLabel, 'CREATIVE ROUND')}`} phase={state.phase} joinCode={state.joinCode} participantCount={state.participantCount}>
