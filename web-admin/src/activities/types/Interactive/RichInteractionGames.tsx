...
const RichStage: React.FC<{ children: React.ReactNode; title: string; kicker: string; phase?: unknown; joinCode?: unknown; joinUrl?: unknown; participantCount?: unknown; roster?: unknown; clock?: React.ReactNode }> = ({ children, title, kicker, phase, joinCode, joinUrl, participantCount, roster, clock }) => (
...
export const DrawingDisplay: React.FC<ActivityComponentProps> = ({ envelope }) => {
  ...
  return <RichStage title={stringOf(config.title, envelope.name || 'Doodle & Guess')} kicker={`🎨 ${stringOf(config.presetLabel, 'DRAWING ROUND')}`} phase={state.phase} joinCode={state.joinCode} joinUrl={state.joinUrl} participantCount={state.participantCount} roster={state.roster} clock={<ActivityStageClock state={state as Record<string, unknown>} />}>
...
export const OrderingDisplay: React.FC<ActivityComponentProps> = ({ envelope }) => {
  ...
  return <RichStage title={stringOf(config.title, envelope.name || 'Order Up')} kicker={`↕ ${stringOf(config.presetLabel, 'ORDERING CHALLENGE')}`} phase={state.phase} joinCode={state.joinCode} joinUrl={state.joinUrl} participantCount={state.participantCount} roster={state.roster} clock={<ActivityStageClock state={state as Record<string, unknown>} />}>
...
export const WordDisplay: React.FC<ActivityComponentProps> = ({ envelope }) => {
  ...
  return <RichStage title={stringOf(config.title, envelope.name || 'Word Storm')} kicker={`☁ ${stringOf(config.presetLabel, 'WORD STORM')}`} phase={state.phase} joinCode={state.joinCode} joinUrl={state.joinUrl} participantCount={state.participantCount} roster={state.roster} clock={<ActivityStageClock state={state as Record<string, unknown>} />}>
...
export const MatchPlayerDisplay: React.FC<ActivityComponentProps> = ({ envelope }) => {
  ...
  return <RichStage title={stringOf(config.title, envelope.name || 'Match Minds')} kicker={`🧠 ${stringOf(config.presetLabel, 'MATCH MINDS')}`} phase={state.phase} joinCode={state.joinCode} joinUrl={state.joinUrl} participantCount={state.participantCount} roster={state.roster} clock={<ActivityStageClock state={state as Record<string, unknown>} />}>
