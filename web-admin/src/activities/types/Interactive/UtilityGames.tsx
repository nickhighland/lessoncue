...
export const UtilityController: React.FC<ActivityComponentProps> = ({ envelope, onCommandSent, hostView }) => {
  ...
  const send = async (action: string, payload?: JsonRecord) => {
    ...
    catch (error) { console.debug('Utility command was rejected; the host notice contains the reason.', error); }
  ...
  const canRun = phase === 'roundIntro' || phase === 'roundComplete';
  const canClear = hasResult || phase === 'reveal' || phase === 'roundComplete';
  const canAdvance = hasResult || phase === 'reveal' || phase === 'roundComplete';
  const canShowLeaderboard = canAdvance;
  ...
  return <div className="act-ctrl-container interactive-host-controller utility-controller">
    ...
    <div className="act-controller-button-row">
      {canStart && <button type="button" className="act-btn act-btn-primary" disabled={busy} onClick={() => void send('start')}>Start utility</button>}
      {canRun && utilityType !== 'mysteryBoxes' && utilityType !== 'countdown' && <button type="button" className="act-btn act-btn-gold" disabled={busy} onClick={() => void send(action, utilityType === 'teamGenerator' ? { teamCount, assignmentMode } : undefined)}>{actionLabel}</button>}
      {canClear && <button type="button" className="act-btn act-btn-secondary" disabled={busy} onClick={() => void send('clear')}>Clear result</button>}
      ...
      {canEnd && <button type="button" className="act-btn act-btn-danger" disabled={busy} onClick={() => void send('finish')}>End utility</button>}
    </div>
    {utilityType === 'countdown' && <div className="act-ctrl-card utility-countdown-controller">
      ...
      <div className="act-controller-button-row">
        {timerRunning ? <button type="button" className="act-btn act-btn-danger" disabled={busy} onClick={() => void send('pausetimer')}>Pause timer</button> : <button type="button" className="act-btn act-btn-primary" disabled={busy || timerRemainingMs <= 0 || phase === 'lobby'} onClick={() => void send(state.timerCompleted === true ? 'clear' : 'resumetimer')}>{state.timerCompleted === true ? 'Reset timer' : 'Start / resume timer'}</button>}
    ...
    {lastResult && <div className="act-ctrl-card utility-controller-result"><span>LAST RESULT</span><strong>{stringOf(lastResult.label, stringOf(lastResult.value, 'Complete'))}</strong><div className="act-controller-button-row"><button type="button" className="act-btn act-btn-secondary" disabled={busy || phase === 'finalResults'} onClick={() => void send('retry', utilityType === 'teamGenerator' ? { teamCount, assignmentMode } : undefined)}>Retry</button><button type="button" className="act-btn act-btn-secondary" disabled={busy || phase === 'finalResults'} onClick={() => void send('skip')}>Skip result</button></div></div>}
  </div>;
};
