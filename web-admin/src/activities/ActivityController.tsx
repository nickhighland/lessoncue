...
const ActivityHostSessionPanel: React.FC<{ hostView: ActivityHostView; onRefresh: () => void }> = ({ hostView, onRefresh }) => {
  ...
  const setParticipantLock = async (participantId: string, displayName: string, locked: boolean) => {
  ...
  const resetPlayers = async () => {
    ...
    setBusyId('resetplayers');
    try { await ActivityApi.executeCommand(hostView.state.runId, { action: 'resetplayers' }); onRefresh(); }
    finally { setBusyId(''); }
  };
