(() => {
    ...
    function linkRow(link, compact) {
        ...
        return '<tr><td><div class="link-main"><span class="link-glyph">' + icon('link') + '</span><div><strong>' + escapeHtml(shortUrl) + '</strong><small>' + escapeHtml(link.title || 'Untitled destination') + '</small></div></div></td><td><div class="destination" title="' + escapeHtml(link.longUrl) + '">' + escapeHtml(link.longUrl) + '</div></td><td><div class="click-count">' + formatNumber(clicks) + '<small>' + formatNumber(nonBots) + ' human</small></div></td>' + (compact ? '' : '<td><span class="muted">' + formatDate(link.dateCreated) + '</span></td>') + '<td><div class="row-actions"><button class="small-action" data-copy-link="' + escapeHtml(shortUrl) + '" title="Copy short link">' + icon('copy') + '</button><button class="small-action danger" data-delete-link="' + escapeHtml(link.shortCode) + '" data-delete-domain="' + escapeHtml(link.domain || '') + '" title="Delete link">' + icon('trash') + '</button></div></td></tr>';
    }
