...
Multi-zone signage stores validated 100 × 100 percentage geometry as JSON so the same responsive layout reaches browsers, Android TV, and Apple TV without platform-specific templates. `SignageWidgetService` resolves only administrator-allowlisted HTTP/HTTPS origins, bounds response size and refresh frequency, normalizes RSS/Atom, ICS, weather/menu, and generic JSON sources, and persists the last successful per-zone cache in SQLite. A failed refresh updates operator-visible status without replacing good cached content. Zone media use synthetic `signage-{id}-zone-{zoneId}` cache identities and the same compatibility selection, checksums, offline cache, telemetry, impact, and deletion-safety paths as ordinary signage media.

The screen manifest keeps `signage` as the ordered active set for backward compatibility and adds `signageSchedule` containing active and future targeted signs. Each attached media payload uses the same selected compatibility or adaptive derivative, checksum, byte count, extension, and offline-eligibility metadata as a lesson cue. Android TV and tvOS add scheduled signs to their existing checksummed caches and telemetry. The browser player stores scheduled signage media in origin Cache Storage, serves matching playback requests through its service worker, prunes obsolete entries, and reports per-item readiness or failure. All clients render image or muted looping-video signage backgrounds, interrupt and resume lesson playback for emergency signs, and return to current signage after a lesson ends. The administration API correlates each synthetic `signage-{id}` item with screen cache inventories to produce per-sign cached, pending, and failed display counts.
