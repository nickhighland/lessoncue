...
`BackupPolicyService` evaluates daily and weekly boundaries in the organization's IANA time zone, serializes runs so only one scheduled copy can execute, encrypts with a Service Admin-supplied password protected by the server key ring, verifies the just-created envelope/manifest/database before reporting success, applies local count-and-age retention only to scheduled records, and optionally streams the encrypted result to an HTTPS WebDAV folder. Its policy and protected WebDAV credential are excluded even when an export deliberately includes other server secrets. Durable attempt, success, verification, filename, and bounded error fields feed the Service Admin alert and Settings status; an hour backoff prevents a broken remote destination from creating a new full archive every polling cycle.
