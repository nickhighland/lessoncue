...
The screen manifest keeps `signage` as the ordered active set for backward compatibility and adds `signageSchedule` containing active and future targeted signs. Each attached media payload uses the same selected compatibility or adaptive derivative, checksum, byte count, extension, and offline-eligibility metadata as a lesson cue. Android TV and tvOS add scheduled signs to their existing caches and telemetry; the browser player prefetches them and supports image or muted looping-video signage backgrounds. The administration API correlates each synthetic `signage-{id}` item with screen cache inventories to produce per-sign cached, pending, and failed display counts.
