...
`HardwareAccelerationService` runs a real 64-pixel H.264 Quick Sync probe at startup and every 24 hours on Linux and Windows. It requires FFmpeg's `h264_qsv` encoder plus a working Intel media driver. Linux enumerates every `/dev/dri/renderD*` node and tries both FFmpeg-documented explicit QSV child-device initialization and QSV derivation from VAAPI; this prevents a non-Intel default adapter from hiding a usable Intel GPU. The initializer that passes the probe is reused unchanged by real conversion jobs, and the selected device, readiness, last use, and last fallback are exposed through the administrator bootstrap. The organization setting is enabled by default but only takes effect when the probe passes. Both compatibility and adaptive workers submit an Intel Quick Sync attempt first, validate the resulting H.264 MP4 with FFprobe, remove an invalid or failed temporary result, and retry the complete conversion through `libx264`. The selected engine is stored on each derivative for operator inspection. Remux-only jobs remain codec-copy operations and are validated through the same output policy.
