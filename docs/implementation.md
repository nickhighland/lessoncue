...
Browser display pairings are intentionally temporary. Every device-authentication check rejects a browser after two hours without a heartbeat, preventing a late reconnect from reviving it. A one-minute cleanup pass removes the screen, its hashed device credential, command history, and diagnostic image, using the credential creation time when a new browser never sends its first status. The platform discriminator limits this behavior to `web-player`; native TV pairings remain durable.
