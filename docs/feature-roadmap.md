...
[x] **Transactional server updates and truthful health checks**
  ...
  [x] **Least-privilege processing of untrusted uploads**
   ...
   - [x] Run FFmpeg, FFprobe, LibreOffice, Poppler, and downloaded-media processing through bounded worker execution. Native Linux uses the unprivileged systemd service boundary, validated data-root writes, local FFmpeg/FFprobe protocols, CPU/address-space/file/process/descriptor/wall-time limits, and bounded output; only the explicitly validated YouTube download path accepts remote media input. Windows combines LocalService/ACL confinement, CPU-time/memory/process/wall-time Job Object limits, and outbound-denied private FFmpeg/FFprobe plus document-converter binaries. Container deployments add read-only root, dropped capabilities, no-new-privileges, bounded memory/CPU/PIDs/tmpfs, and the same worker.
