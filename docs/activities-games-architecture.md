...
| --- | --- | --- |
| Activity library | `web-admin/src/activities/ActivityLibrary.tsx` and `activityRegistry.ts` | Keep as the single selector. It now provides persistent grid/list views, search and capability filters, favorites, thumbnails, multi-select, bulk delete/archive/restore/duplicate actions, dependency-aware destructive messaging, archived recovery, manual ordering backed by `LibraryPosition`, and client-only draft previews; registry metadata will continue to grow for categories, modes, requirements, and presets. |
