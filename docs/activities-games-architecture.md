...
16. Preset authoring: `activityPresetRegistry.ts` supplies editable starter templates for the Quiz, Poll, Buzzer, Creative, and Bluffing engines. The selected template is saved as `config.preset` plus the definition `PresetType`, while the server continues to run the existing engine/type and keeps preset metadata out of runtime branching.
